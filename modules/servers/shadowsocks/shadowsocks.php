...
function shadowsocks_mysql($params) {
	$mysql = 
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);

	if (!$mysql) {
		$mysql = "Can not connect to MySQL Server".mysql_error();
	} else {
		mysql_select_db($params['configoption1'],$mysql);
		$add = mysql_query("alter table user add pid varchar(50) not 
null",$mysql);
		if (!$add) {
			$mysql = "Can't add.";
		}
...
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);
	...
	if (!$mysql) {
		...
		if (!$select == "") {
			$get_last_port = mysql_query("SELECT port FROM user order by 
port desc limit 1",$mysql);
...
function shadowsocks_CreateAccount($params) {
	$serviceid			= $params["serviceid"]; # Unique ID of the 
product/service in the WHMCS Database
    $pid 				= $params["pid"]; # Product/Service ID
    $producttype		= $params["producttype"]; # Product Type: 
hostingaccount, reselleraccount, server or other
    $domain 			= $params["domain"];
  	$username 			= $params["username"];
  	$password 			= $params["password"];
    $clientsdetails 	= $params["clientsdetails"]; # Array of clients details - 
firstname, lastname, email, country, etc...
    $customfields 		= $params["customfields"]; # Array of custom field 
values for the product
    $configoptions 		= $params["configoptions"]; # Array of configurable 
option values for the product
    ...
    $serversecure 		= $params["serversecure"]; # If set, SSL Mode is 
enabled in the server config
  	$adminusername		= mysql_fetch_array(mysql_query("SELECT username FROM 
`tbladmins`"));
  	$mysql 				= 
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);
	$port 				= shadowsocks_CreateNewPort($params);
  	...
  	if (!$mysql) {
  		...
  		mysql_select_db($params['configoption1'],$mysql);
  		$select = mysql_query("SELECT pid FROM user WHERE 
pid='".$serviceid."'",$mysql);
  		$select = mysql_fetch_array($select);
...
password='".$results['password']."' where id='".$params["serviceid"]."'");
			...
			if(isset($params['configoptions']['traffic'])) {
				$traffic = 
$params['configoptions']['traffic']*1024*1048576;
				$query = mysql_query("INSERT INTO 
user(pid,passwd,port,transfer_enable) VALUES 
('".$params['serviceid']."','".$password."','".$port."','".$traffic."')",$mysql);
				if ($query) {
					$result = 'success';
				} else {
					// $result = 'Error.Cloud not create a new 
user.';
					$result = mysql_error();
				}
			} else {
...
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);
	...
	if (!$mysql) {
		...
		mysql_select_db($params['configoption1'],$mysql);
		if(mysql_query("DELETE FROM user WHERE 
pid='".$params['serviceid']."'",$mysql)) {
			$result = 'success';
		} else {
			$result = 'Error. Cloud not Terminate this 
Account.'.mysql_error();
		}
	}
	return $result;
...
pid='".$params['serviceid']."'",$mysql);
		...
		if ($select == "") {
			...
			if (mysql_query("UPDATE user SET  passwd='".$password."' 
WHERE pid='".$params['serviceid']."'",$mysql)) {
				$result = 'success';
			} else {
...
pid='".$params['serviceid']."'",$mysql);
		...
		if ($select == "") {
			...
			if (mysql_query("UPDATE user SET passwd='".$password."' WHERE 
pid='".$params['serviceid']."'",$mysql)) {
				$result =  'success';
			} else {
...
passwd='".$params['password']."' WHERE pid='".$params['serviceid']."'")) {
				...
				$where = 
array("fieldid"=>$fieldid,"relid"=>$params["serviceid"]);
				update_query($table,$update,$where);
				$result = 'success';
			} else {
...
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);
	if (!$mysql) {
		...
		if(isset($params['configoptions']['traffic'])) {
			...
			if (mysql_query("UPDATE user SET 
transfer_enable='".$traffic."' WHERE pid='".$params['serviceid']."'",$mysql)) {
				return 'success';
			} else {
				return mysql_error();
			}
...
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);
	if(!$mysql) {
		...
		mysql_select_db($params['configoption1'],$mysql);
		$query = mysql_query("UPDATE user SET u='0',d='0' WHERE 
pid='".$params['serviceid']."'",$mysql);
		if ($query) {
			return 'success';
		} else {
...
function shadowsocks_ClientArea($params) {
	...
	$mysql = 
mysqli_connect($params['serverip'],$params['serverusername'],$params['serverpassword'],$params['configoption1']);
	if(mysqli_connect_errno($mysql)) {
		...
		//Usage
		$row = mysqli_query($mysql,"SELECT sum(u+d),port,passwd FROM user 
WHERE pid='".$params['serviceid']."'");
		$Query = mysqli_fetch_array($row,MYSQLI_BOTH);
		$Usage = $Query[0] / 1073741824;
		//Port 1048576 1073741824
		// $QueryPort = mysql_query("SELECT port FROM user WHERE 
pid='".$params['pid']."'");
...
mysql_connect($params['serverip'],$params['serverusername'],$params['serverpassword']);
	if(!$mysql) {
		...
		//Traffic
		$traffic = null;
		// $traffic = isset($params['configoptions']['traffic']) ? 
$params['configoptions']['traffic']*1024 : isset($params['configoption5']) ? 
$params['configoption5']*1024 : 1048576;
		if (isset($params['configoptions']['traffic'])) {
		...
		//Usage
		$Query = mysql_query("SELECT sum(u+d),port FROM user WHERE 
pid='".$params['serviceid']."'",$mysql);
